...
**Last updated:** 2026-08-15 (F1 merged after review + fix round; C2 data spike is next — human gate at its end)
...
| C2 data spike | ⏭ **next — HUMAN GATE at its end** | Runs against the operator's LOCAL token (`backend/.ind_money_token.json`, refresh-token auto-renew). Note: the prod link died with the C1 Space restart (expected post-C0) — irrelevant to C2, reconnect needed later for D0's real-snapshot option / S1. M1 must not start until a human reads `docs/ind_money_payloads.md`'s go/no-go. |
