...
| A1 AI overview | ✅ **built 2026-08-16 — awaiting review** | Branch `feat/ai-overview` (off `main` @ `7ae2eb2`). **Numbers computed in Python (`agents/portfolio/metrics.py`), agents narrate by `[[token]]` only — a number can only come from a computed `Metric.display`, enforced **structurally** (`invented_figures` flags any digit in a free-prose text segment — numbers reach the reader only as metric chips), with a scripted-prose fallback if a model ever slips a numeral.** No XIRR (simple `pnl_pct`, shown only with a cost basis). Multi-agent graph (`graph/portfolio_graph.py`): 4 parallel specialists → synthesizer, **no gate/guardrail/interrupt/checkpointer**, invoked with `portfolio_runnable_config()` (tracing off, asserted). `POST /portfolio/overview` streams `start`/`update`/`complete` (same identity + typed source-error contract as `/portfolio/*`). **Graceful degradation is a hard requirement and is met:** no key / provider down / rate-limited / over the in-code daily spend ceilings → the dashboard renders every computed number and the panel shows "AI overview unavailable", never an error (pinned with the key removed). `get_chat_llm` gained `provider=` which **wins over env** — portfolio → OpenAI (pinned to api.openai.com), the 4 Lab agents **untouched** and stay on Groq even with `OPENAI_COMPATIBLE_MODEL` set (compat now requires `OPENAI_BASE_URL`). Frontend AI overview panel per `a2-overview.html` (inline metric chips, agent chips, computed-metrics rail, degraded state). Static `/demo` artifact committed (`backend/tests/fixtures/demo/overview.json`, regen via `python -m agents.portfolio.demo`). **Adversarial review round applied:** structural (not substring) anti-fabrication detector, synthesizer prompt also routed through `redact()`, integration test that `redact()` is actually called, `overview_stream` tracing-off assertion, global-spend-cap test, per-request timeout — all 4 named guards mutation-checked. 516 pytest + 66 vitest + tsc/build green; **1** real OpenAI call spent (gated live test); degraded + narrative screenshots verified (scratchpad). Docs: `docs/SPECS/A1.md`, `docs/TESTING/A1.md`. **Operator (MORNING.md): set an OpenAI provider-side budget cap; clear the stray local `OPENAI_COMPATIBLE_MODEL` (currently inert but must stay unset in prod); `OPENAI_API_KEY` as a Space secret.** |
| U1 app shell | queued | |
