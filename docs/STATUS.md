...
| #18 Lab on the product theme | ✅ **done 2026-09-11** | Five-way bake-off (`docs/design/lab/`, same synthetic run in each, all in the locked language); operator picked **A — Desk Console**, losers in `lab/rejected/`. DECISION gains a **"The Lab joins the direction"** section + two amendments (`--adp-bad-*`, a REJECT tint; `--adp-warn-mark`, a mid amber — a mark, never text). Lab renders inside `[data-adp]` with `ThemeBootstrap`, so Portfolio ↔ Lab holds one theme; terminal token set **deleted** from `globals.css` + `tailwind.config.ts` (`up/down/flag/cyan`, `.eyebrow`, `.pill-*`, `.hero-glow`, `.caret`, `.bg-up-soft`); `TopBar`/`AuthButton`/`AgentStepCard`/`ui/button`/`ui/card` removed; `portfolio/ui.tsx` → `ui/adp.tsx` as the one primitive set; Lab components → `components/lab/`. Simulation band + not-advice footer unchanged in prominence. Also fixed the pre-paint hydration warning on all four themed layouts (pre-existing, inherited). 167 vitest, `next build` green. |
...
| L1 pre-launch bar | ✅ **done 2026-08-16 — INVITE GATE, all build cards complete** | Merged (2 commits). Privacy/terms name all 7 subprocessors (Groq/OpenAI/LangSmith/Clerk/Neon/HF/Vercel), reachable from every page's footer incl. /sign-in + /waitlist; unskippable consent-at-link; **`DELETE /account` atomic** — revoke upstream first, then one `DELETE FROM users` cascading all 7 tables + per-user in-memory caches, exhaustive zero-rows test (a half-deleted user is impossible by construction); rate-limit 429 inside CORS + OPTIONS-exempt; global+per-user spend caps; **C0 interim admin path fully removed** (verified dead with the secret still set); **`NEXT_PUBLIC_AUTH_ENABLED` flipped ON**. Ultracode 3-lens review: deletion PASS, consent/admin/flag PASS, legal FAIL (LangSmith undisclosed + footer gaps) → fixed & re-verified; 673 pytest green under random + fixed ordering (flake root-caused), 2 mutations killed. |

## Post-launch backlog cards
