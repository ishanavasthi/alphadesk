...
**Last updated:** 2026-08-16 (F3 built + review round folded in on `v2-foundation` — the auth singleton is dead, links are per-user and encrypted, F2's three latent Importants closed; awaiting review. Operator: four env vars + a real IND Money login, `docs/TESTING/F3.md` §6)
...
| F3 per-user linking | ✅ **done 2026-08-16 — on `v2-foundation`, awaiting review** | The `_Auth` singleton is gone. Every IND Money credential is now a Fernet-encrypted `broker_links` row per `(user, source)`, hydrated by `AuthStore.for_user()` with a **per-user** `asyncio.Lock` (a shared one made every user wait on the slowest broker round-trip); refresh/expiry logic carried over verbatim. OAuth `state` moves from an in-memory dict to `oauth_pending`: `/auth/callback` resolves the owner from the state **alone** (no cookie, no header), single-use via `DELETE … RETURNING` (a SELECT-then-DELETE would let two racing callbacks both win) and a 10-min TTL, both enforced on read. Both scopes requested (`portfolio:read market:read`); the registered client is stored and **reused** on re-link, re-registering only when the redirect URI moved — migration `0004` adds `broker_links.redirect_uri` for exactly that. Unlink calls the discovery doc's `revocation_endpoint` **before** deleting, and reports `revoked_upstream` when the upstream call fails. **Gates:** `/auth/login|logout` are JWT-only (an admin-header link is an unowned link — the thing C0 existed to stop); `/auth/status` answers for the caller and hands an anonymous visitor nothing (it used to tell the internet whether the operator was connected); `/portfolio/*` takes a Clerk JWT **or**, until L1, the C0 admin secret — one marked function (`_admin_identity`) plus one parameter, deleted by unsetting `ALPHADESK_ADMIN_SECRET`. A *bad* token never falls through to the admin path. **F2's three latent Importants are closed:** `CLERK_AUTHORIZED_PARTIES` is mandatory (unset ⇒ 503), `sid` is required so an instance JWT-template token cannot authenticate, and unknown `kid`s can no longer drive 1:1 outbound JWKS fetches (own key-set cache + 30 s cooldown; a test fires 50 random kids and pins the cost at **+1**). Plus: verification is its own dependency resolved **before** the session (a bad token 401s instead of 500ing on a dead Postgres), and a reachable-but-unparseable JWKS is 503. **Adoption** of the pre-F3 `user_id="local"` rows (the broker link **and every S1 snapshot day** — a point-in-time source, so a lost day is lost) runs only for `ALPHADESK_OPERATOR_EMAIL`, matched against the **verified primary** address via the Clerk Backend API; never first-comer, idempotent, collisions never overwrite the signed-in user's own rows. Connector is per user (`get_connector(user_id)`), the nightly job builds one per linked user, and the M1/S1 carry is fixed: `status_verified()` now surfaces `revoked`, so `LinkHealth.REVOKED` is reachable on an idle poll instead of flattening to `NEEDS_RELINK`. **613 pytest (103 new), 63 vitest (7 new), tsc + build clean; nine named mutations, 9/9 killed.** Live: two real Clerk users signed in through the real UI on `leading-sheepdog-6215`, **25/25 checks** against the real backend + Postgres, with only the IND Money OAuth endpoints mocked (`/register` mints a live client, `/token` burns a code, `/revoke` kills a grant). That run **found a real bug** no unit test had: `/auth/status` — the first call a signed-in browser makes — read the token without registering the user, so adoption never fired; all four entry points now go through `deps.register_identity` and the regression is pinned. Facts worth keeping: a **browser**-minted Clerk token carries `azp` = the page origin and `sid`; a **Backend-API**-minted one carries no `azp` and is (correctly) rejected; session tokens live ~60 s; a new device triggers an email code, so live runs need `+clerk_test` addresses (code `424242`); and `clerkMiddleware()` needs `CLERK_SECRET_KEY` at runtime on the *frontend* host or every route 500s with the flag on. Flag-off pixel check on `/portfolio` (operator config): with the live Refresh-countdown band masked, **0 differing px of 2,239,200** (`bbox=None`) against a pre-F3 build; bundle sizes and shared chunk hashes unchanged. **Operator wiring blocks nothing in this card but blocks the real link:** `TOKEN_ENCRYPTION_KEY` is **currently unset** (F1 added it; F3 is the first card that stores an encrypted credential), plus `CLERK_AUTHORIZED_PARTIES`, `ALPHADESK_OPERATOR_EMAIL`, `CLERK_SECRET_KEY`, and `alembic upgrade head` (0003→0004). Morning checklist: `docs/TESTING/F3.md` §6. Docs: `docs/SPECS/F3.md`, `docs/TESTING/F3.md`. **Adversarial review round folded in (`74cd131`):** reflected XSS on `/auth/callback?error=` (the param was interpolated unescaped — escaping moved into `_auth_html` so no future branch can forget); adoption trusted a matching *email claim*, which a JWT template can emit unverified — the claim is now a negative pre-filter only and Clerk's verified primary address decides, closing a self-service takeover of the operator's links, snapshots **and** `operator_user_id()`; `operator_user_id` matched with `ILIKE`, making `_`/`%` in `ALPHADESK_OPERATOR_EMAIL` wildcards → `lower(email) = …`; a transient Clerk failure was cached as a settled 'not the operator', so one blip during the operator's first sign-in stranded their history for the process lifetime → only settled answers are remembered; an undecryptable `broker_links` row (rotated `TOKEN_ENCRYPTION_KEY`) 500'd out of `/auth/*` and `/portfolio/*` → now `needs_relink` with the row left intact, and a keyless server refuses at `/auth/login` in words; the interim admin path was **three** sites (an inline `compare_digest` in `_status_identity` that no removal note mentioned) → one `admin_secret_accepted()` plus an eight-row L1 removal checklist in SPECS §5, with `/auth/status` accepting the header stated as deliberate rather than silent; the DCR re-register fallback the docstring claimed is now implemented (`invalid_client` → `forget_client()` → next Connect re-registers; `invalid_grant` leaves it alone) — this is the failure mode the morning real login is most likely to hit; store/lock caches bounded (they hold decrypted tokens). Two test gaps the review proved by mutation are closed: logout ordering (revoke-before-delete now asserted by recorded order) and the per-user connector in the batch snapshot path (the factory's connectors refuse any other user, so `connector_factory("local")` fails). `/analyze` stays ungated and ambient **by orchestrator ruling** — F4 owns it; a pointer comment marks it. |
