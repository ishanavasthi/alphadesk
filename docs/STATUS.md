...
**Last updated:** 2026-08-16 (F4 built on `v2-foundation` — per-user Lab state, the last ambient path (`/analyze`) closed, the paper watchlist persists (migration 0005), the desk moved to `/lab`; awaiting review. Operator: Neon `DATABASE_URL` + `alembic upgrade head` for watchlist durability, and one real IND-backed `/analyze`, `docs/TESTING/F4.md`)
...
| F4 per-user Lab state | ✅ **done 2026-08-16 — awaiting review** | `PortfolioState.user_id`; `_RUNS`/`_ANALYSES`/`_ACTIONS` keyed by user with a `_owned_run` 404-not-403 ownership check on `/approve`+`/analysis/{id}`+`/status/{id}`, `/analyses` filtered to caller. **Last ambient path closed:** `/analyze` takes `_lab_identity` (JWT-or-single-tenant-`local`, **no** admin path), gates on *that user's* link (401 without identity, 409 unlinked), and binds the caller as the run identity via a `ContextVar` so the pipeline's userless MCP tools mint from their own `AuthStore` (reset in `finally`; `/approve` binds around its resume too). `ambient_user_id()` kept — only `tools/ind_money.py`'s market-data tools use it now; F3 §11.0 ledger line struck. **Watchlist persists** (migration 0005, `watchlist` table, PK `(user_id, symbol)`, denormalized decision record + opaque non-FK `run_id`, DB-level cascade `c`) with in-memory per-user fallback when no DB; "view original run" degrades to 404 "no longer available". Desk **moved to `/lab`** (+`/lab/a/[id]`; `/`→`/lab` redirect) with a persistent "live simulation / not investment advice / no orders" banner in `app/lab/layout.tsx`; terminal aesthetic untouched. 625 pytest (+12), 64 vitest (+1 lab-label), tsc+build clean, migration idempotent + drift-free, `/lab` visually verified 1440px. **Morning: one real IND-backed `/analyze` (single-tenant dev) is the only unmocked check; needs Neon `DATABASE_URL` + `alembic upgrade head` for watchlist persistence on the Space.** Docs: `docs/SPECS/F4.md`, `docs/TESTING/F4.md`. |
