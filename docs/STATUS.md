...
**Last updated:** 2026-08-16 (L1 built on `feat/prelaunch` — the invite gate: delete-my-data cascade (revoke upstream → `DELETE FROM users` wipes every table, proved exhaustively) + consent-at-link-time + real privacy/terms + rate-limit 429s + the F3 §5 admin removal executed + `NEXT_PUBLIC_AUTH_ENABLED` flipped on; awaiting operator wiring. Operator: Clerk Waitlist + real keys + unset `ALPHADESK_ADMIN_SECRET` on the Space + real IND login, `docs/MORNING.md`)
...
| U1 app shell | ✅ **done 2026-08-16** | Merged (7 commits). 8 routes, single shell owner (F3 TerminalChrome interim conditional retired — terminal chrome on /lab, shadcn on product surfaces); **public /demo makes 0 LLM/auth/network calls — structurally guaranteed** (no code path to the network exists; renders with backend down; serves A1's committed artifact + M1 stub fixtures); unmissable sticky sample-data banner; signed-in routing (linked→/portfolio, unlinked→gate, anon→landing) live-verified w/ real Clerk session; /privacy+/terms placeholders (L1 fills); AuthProvider de-globalized (every useIndMoney consumer covered). /portfolio+/lab pixel-unchanged; flag-off ships no Clerk. Review PASS: 0 Critical/Important. 664 pytest / 71 vitest. |
| L1 pre-launch bar | ✅ **built 2026-08-16 on `feat/prelaunch` — awaiting operator wiring; INVITE GATE** | Delete-my-data (`DELETE /account`, JWT): revoke upstream first (reuse F3's `AuthStore.logout`), then a single `DELETE FROM users` cascades **every** table (proved by an exhaustive test that seeds all 7 tables + in-memory Lab state and asserts zero rows survive + revocation called), then purge in-memory Lab state / connector / seen-user / adoption caches; frontend "Delete my data" in the Clerk account menu (type-to-confirm → signout). Consent-at-link-time: a shared unskippable consent screen (`components/consent`) every Connect entry point routes through, naming exactly what is read (holdings, values, SIPs, net-worth totals) and what never is (no trading, no credentials — OAuth only). Real `/privacy`+`/terms`: subprocessors named (Groq+OpenAI+Clerk+Neon+Hugging Face+Vercel), retention (snapshots kept, raw pruned 90d), no-orders, not-advice, DPDP deletion; footer reaches every page (added to Lab). Rate-limit middleware: per-caller + global fixed-window ceilings on `/analyze`,`/portfolio/overview`,`/auth/login` → 429 (A1's overview *spend* cap still **degrades**, unchanged — the 429 is the request-rate cap). **F3 §5 executed: all 6 backend admin sites + the frontend `ADMIN_SECRET` deleted; no admin-header path authenticates anything now** (`ALPHADESK_ADMIN_SECRET` unset changes nothing — pinned). `NEXT_PUBLIC_AUTH_ENABLED` flipped on via committed `frontend/.env.production`; flag-on production build verified clean (9 routes, middleware). 670 pytest (15 new: exhaustive cascade delete, rate-limit 429s, §5 removal checklist) / 85 vitest (14 new: consent, delete, policy), tsc clean. **Operator wiring (blocks going live): enable Clerk Waitlist mode + approve first users, set real Clerk keys on Vercel + Space, unset `ALPHADESK_ADMIN_SECRET` on the Space, real IND login — see `docs/MORNING.md`.** Docs: `docs/SPECS/L1.md`, `docs/TESTING/L1.md`. |
