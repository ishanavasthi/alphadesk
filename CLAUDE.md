...
AlphaDesk is a multi-agent Indian-equity research desk: a LangGraph pipeline (FastAPI backend) that scans NSE movers, researches candidates, writes analyst reports, enforces risk guardrails, and pauses for human approval before adding stocks to a paper watchlist. Market data comes from the read-only IND Money MCP server; reasoning runs on Groq (or any OpenAI-compatible endpoint). The frontend is a Next.js app on one locked design language (`docs/design/DECISION.md`): shadcn/zinc, light and dark, across the dashboard, the marketing pages and the Lab. **No real orders are ever placed** — the broker layer is a stub.
...
| --- | --- | --- | --- |
| AI Overview (`agents/portfolio/*`) | `OVERVIEW_PROVIDER` | `OVERVIEW_MODEL` (legacy alias `OPENAI_OVERVIEW_MODEL`) | real OpenAI, `gpt-4o-mini` |
| Lab simulation (scanner/research/analyst/risk_manager) | `LAB_PROVIDER` | `LAB_MODEL` blanket; `LAB_<AGENT>_MODEL` per agent | Groq, historical per-agent tiers |

Agents never call `get_chat_llm` directly: the overview goes through `get_overview_llm()` and each Lab agent through `get_lab_llm("<agent>", DEFAULT_MODEL)`, which apply that family's vars. Routing precedence is (1) an explicit `provider=` argument, (2) the family's own env var, (3) the ambient default. **The A1 invariant survives:** rerouting a family takes *that family's* variable, set deliberately — the stray v1 leftovers `OPENAI_BASE_URL` / `OPENAI_COMPATIBLE_MODEL` still move nothing on their own (the overview's fallback is the explicit `provider="openai"`, which pins `api.openai.com`; the Lab's default path enables compat **only** on a real `OPENAI_BASE_URL`, so a lone `OPENAI_COMPATIBLE_MODEL` stays inert). Per plan §9 both stay **unset** in prod. `provider="openrouter"` pins `openrouter.ai/api/v1` and `OPENROUTER_API_KEY` — it never falls back to `OPENAI_API_KEY`, so a real OpenAI key is never sent to a third party; a missing key raises. An unrecognized provider string raises rather than silently billing the wrong vendor.

**Structured output must go through `agents.llm.structured(llm, schema)`**, which pins `method="function_calling"`. langchain-openai defaults to strict `json_schema`, which many OpenRouter/compat models do not implement — they answer in prose, the Lab callers swallow the parse error as "skip this stock", and a run comes back empty with no error logged anywhere. A vitest-style source scan (`tests/test_llm_routing.py`) fails if any agent calls `with_structured_output` directly. Any model you point the Lab at must support tool-calling.
...
**One design language, the Lab included (#18).** There is no second theme. `docs/design/DECISION.md` is the binding contract and `frontend/app/portfolio/portfolio.css` its token file — named for the surface that introduced it, imported by all of them. Every themed layout wraps itself in `id="adp-root" data-adp` + `<ThemeBootstrap/>`, which is what scopes the palette, lets `ui/dialog` portal inside it, and keeps one theme across a client-side route change; `frontend/tests/theme-continuity.test.tsx` scans for `data-adp` and fails a surface that opts in without the wiring, so its hard-coded surface list must grow with any new one. The Lab's **dark Bloomberg terminal is gone** as of #18 — `globals.css` no longer carries a second token set, and `up`/`down`/`flag`/`cyan`, `.eyebrow`, `.pill-*`, `.hero-glow`, `.caret` and `.bg-up-soft` went with it. Status colour is read straight off the tokens (`text-[var(--adp-good)]`), never a Tailwind colour name. Lab components live in `frontend/components/lab/`, the shared DECISION primitives in `frontend/components/ui/adp.tsx`. The unmissable simulation band and the not-advice footer survive the restyle unchanged in prominence. Reference: `docs/design/lab/a-console.html`.

**Postgres (`backend/db/`, added by F1; first consumer S1).** SQLModel tables — `users`, `broker_links`, `oauth_pending` (F1), `snapshot_days`, `snapshot_holdings`, `snapshot_raw` (S1), `watchlist` (F4, migration 0005), `portfolio_cache` (migration 0006) and `manual_fds` (B10, migration 0008) — all with DB-level `ON DELETE CASCADE`, an async engine + `async_session` FastAPI dependency, Fernet helpers for the `*_enc` columns (`TOKEN_ENCRYPTION_KEY`), and Alembic migrations run on asyncpg — there is deliberately no sync Postgres driver. Details in `docs/SPECS/F1.md` and `docs/SPECS/S1.md`. **`broker_links` and `oauth_pending` got their first real consumer in F3**, which also added `broker_links.redirect_uri` (migration 0004) — a dynamically-registered OAuth client is bound to its redirect URI, so a stored client stops working the moment the callback URL moves. **The database is optional at runtime:** with `DATABASE_URL` unset the dashboard still serves live totals and history is honestly empty — never a 500. Any deploy shipping a new migration needs `alembic upgrade head` against the production URL.

**Manual fixed deposits (B10).** The vendor's FD numbers are wrong at the source (#65), so users enter FDs themselves: `manual_fds` rows hold only the *terms* (label, principal, rate, compounding, start/maturity) and `backend/services/manual_fd.py` computes the accrued value — pure `Decimal`, actual/365, clamped before start and frozen at maturity — on **every read**; nothing derived is stored. CRUD at `/portfolio/fds` (`backend/api/routes/manual_fd.py`, portfolio identity, other users' ids are 404; no-DB: reads degrade to empty, writes 503 — deliberately *not* the watchlist's in-memory fallback, a durable record must never silently evaporate). Manual rows merge **additively**: appended to `GET /holdings?asset_type=FD` after the cache (never cached), and `/summary` gains an additive `manual` block — vendor-derived fields and the daily snapshots stay byte-untouched. The **history trend** prices them too: `/portfolio/history` (`history_points`) adds each deposit's accrued value *computed at each captured day* from its stored terms — never stored, so entering or editing an FD re-prices the whole line on the next read; days before `start_date` contribute nothing and past maturity the frozen maturity value rides every later point. A failed manual read costs the FD contribution, never the chart. Frontend card + dialogs in `frontend/components/portfolio/ManualFds.tsx`. Details: `docs/SPECS/B10.md`.
